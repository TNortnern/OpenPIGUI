...
import { decodeRightRailPreferences } from "../src/right-rail-model";
import {
  applyTimelineEvent,
...
export class DesktopAppStore implements AppStoreInternals {
  ...
  async setRightRailState(rightRail: RightRailPreferences): Promise<DesktopAppState> {
    await this.initialize();
    const normalized = decodeRightRailPreferences(rightRail);
    this.state = {
      ...this.state,
      rightRail: normalized,
      lastError: undefined,
      revision: this.state.revision + 1,
    };
  ...
  private async initializeInternal(): Promise<void> {
    ...
    try {
      this.state = {
        ...
        rightRail: persisted.rightRail ?? this.state.rightRail,
  ...
  async persistUiState(): Promise<void> {
    ...
    const payload: PersistedUiState = {
      ...
      rightRail: this.state.rightRail,
      lastViewedAtBySession: mapToRecord(this.sessionState.lastViewedAtBySession),
      pinnedAtBySession: mapToRecord(this.sessionState.pinnedAtBySession),
