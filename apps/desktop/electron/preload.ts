...
contextBridge.exposeInMainWorld("piApp", {
  ...
  setRightRailState: (preferences: RightRailPreferences) =>
    ipcRenderer.invoke(desktopIpc.setRightRailState, preferences) as Promise<DesktopAppState>,
  ...
  ensureBrowserPanel: (target: BrowserTarget) =>
    ipcRenderer.invoke(desktopIpc.browserEnsure, target) as Promise<BrowserStateSnapshot>,
  navigateBrowser: (input: BrowserNavigateInput) =>
    ipcRenderer.invoke(desktopIpc.browserNavigate, input) as Promise<BrowserStateSnapshot>,
  browserBack: (target: BrowserTarget) =>
    ipcRenderer.invoke(desktopIpc.browserBack, target) as Promise<BrowserStateSnapshot>,
  browserForward: (target: BrowserTarget) =>
    ipcRenderer.invoke(desktopIpc.browserForward, target) as Promise<BrowserStateSnapshot>,
  browserReload: (target: BrowserTarget) =>
    ipcRenderer.invoke(desktopIpc.browserReload, target) as Promise<BrowserStateSnapshot>,
  browserStop: (target: BrowserTarget) =>
  ...
  setBrowserVisible: (visible: boolean, target?: BrowserTarget) =>
    ipcRenderer.invoke(desktopIpc.browserSetVisible, visible, target) as Promise<void>,
  getBrowserState: () =>
    ipcRenderer.invoke(desktopIpc.browserGetState) as Promise<BrowserStateSnapshot | null>,
  openBrowserExternal: () =>
    ipcRenderer.invoke(desktopIpc.browserOpenExternal) as Promise<void>,
  destroyBrowser: () =>
    ipcRenderer.invoke(desktopIpc.browserDestroy) as Promise<void>,
  onBrowserStateChanged: (listener: (state: BrowserStateSnapshot) => void) =>
