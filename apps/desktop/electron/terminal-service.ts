...
export class TerminalService {
  ...
  private spawnPty(webContents: WebContents, session: TerminalSession): void {
    try {
      ...
      session.pty = loadNodePty().spawn(session.shell, interactiveShellArgs(session.shell), {
        name: "xterm-256color",
...
function interactiveShellArgs(shellPath: string): string[] {
  ...
  if (!process.env.PI_APP_TEST_MODE) return ["-i"];
  // Core tests prove terminal behavior, not the machine owner's potentially
  // slow or interactive shell startup files. Keep that surface deterministic.
