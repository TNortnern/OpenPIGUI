...
type PiAppTestHooks = {
  inspectBrowserPanel?: (ownerWebContentsId: number) => BrowserPanelInspection | null;
  executeInBrowserPanel?: (ownerWebContentsId: number, code: string) => Promise<unknown>;
};

/** Inspect the main-process WebContentsView for the first app window (test mode hooks). */
export async function inspectBrowserPanel(harness: DesktopHarness): Promise<BrowserPanelInspection | null> {
  return harness.electronApp.evaluate(({ BrowserWindow }) => {
    const hooks = (globalThis as { __PI_APP_TEST_HOOKS?: PiAppTestHooks }).__PI_APP_TEST_HOOKS;
    const window = BrowserWindow.getAllWindows().find((candidate) => !candidate.isDestroyed());
    if (!window || !hooks?.inspectBrowserPanel) {
      return null;
    }
    return hooks.inspectBrowserPanel(window.webContents.id);
  });
}

/** Execute JavaScript inside the managed browser WebContentsView (not the app renderer). */
export async function executeInBrowserPanel(harness: DesktopHarness, code: string): Promise<unknown> {
  return harness.electronApp.evaluate(async ({ BrowserWindow }, script) => {
    const hooks = (globalThis as { __PI_APP_TEST_HOOKS?: PiAppTestHooks }).__PI_APP_TEST_HOOKS;
    const window = BrowserWindow.getAllWindows().find((candidate) => !candidate.isDestroyed());
    if (!window || !hooks?.executeInBrowserPanel) {
      throw new Error("Browser panel execute hook is unavailable");
    }
    return hooks.executeInBrowserPanel(window.webContents.id, script);
  }, code);
}

/** Count non-destroyed BrowserWindows (used to prove popups do not spawn new windows). */
export async function countAppBrowserWindows(harness: DesktopHarness): Promise<number> {
  return harness.electronApp.evaluate(({ BrowserWindow }) =>
    BrowserWindow.getAllWindows().filter((window) => !window.isDestroyed()).length,
  );
}

export async function getBrowserStateFromRenderer(window: Page) {
  return window.evaluate(async () => {
    const app = (window as PiAppWindow).piApp;
    if (!app?.getBrowserState) {
      return null;
    }
    return app.getBrowserState();
  });
}
