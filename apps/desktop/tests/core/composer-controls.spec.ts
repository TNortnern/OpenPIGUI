...
test("supports keyboard shortcuts, slash menus, and topbar controls through the user surface", async () => {
  ...
  try {
    ...
    await expect(window.locator(".view-header__title")).toContainText("Providers");
...
test("dark mode keeps the send button visible before and after typing", async () => {
  ...
  try {
    ...
    await settingsSurface.getByRole("button", { name: "General", exact: true }).click();
    await expect(window.locator(".view-header__title")).toHaveText("General");
    await settingsSurface.getByRole("radiogroup", { name: "Theme" }).getByRole("radio", { name: "Dark", exact: true }).click();
    await expect
