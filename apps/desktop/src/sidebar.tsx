...
function ThemeModeControl({
  ...
  const detail =
    themeMode === "system" ? `matches ${resolvedTheme}` : themeMode === "light" ? "always light" : "always dark";
  const nextLabel = nextMode === "system" ? "System" : nextMode === "light" ? "Light" : "Dark";
  const Icon = themeMode === "system" ? SystemThemeIcon : themeMode === "light" ? SunIcon : MoonIcon;
  ...
  return (
    ...
    >
      <Icon />
      <span className="sidebar__theme-toggle__copy">
        <span className="sidebar__theme-toggle__label">{label}</span>
        <span className="sidebar__theme-toggle__detail">{detail}</span>
      </span>
    </button>
  );
}
